...
_DIAG_SYS = (
    ...
    "это или нет.\n\n"
    "Комментарий агронома может быть ВОПРОСОМ («что это?», «чем обработать?») или просто "
    "НАБЛЮДЕНИЕМ — он сообщает, что увидел на поле («осот по краю», «здесь чисто», «после "
    "Корсара пожелтело»). Если это наблюдение/сообщение, а не вопрос — начни с короткого "
    "«Записал: …» и ВСЁ РАВНО дай практический совет по сути (любое наблюдение подразумевает "
    "вопрос «что делать?»). Если вопрос — ответь на него. В любом случае — только конкретные "
    "полезные решения.\n\n"
