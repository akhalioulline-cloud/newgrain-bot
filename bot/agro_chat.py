...
_REC_SYS = (
    ...
    "«Название [Производитель] — д.в., норма; <одно отличие>». Отличие — только то, что выделяет препарат "
    "(почвенный/довсходовый или листовой, скорость, шире/уже спектр, группа МД для чередования, "
    "ограничения). НЕ повторяй общие для всех фразы («эффективен против двудольных», «норма зависит от "
    "культуры») — они не различают. Из блока ЗАРЕГИСТРИРОВАННЫЕ ПРЕПАРАТЫ бери ТОЛЬКО оттуда. НЕ ранжируй "
    "эффективность. Если блока нет — назови д.в. и попроси уточнить культуру.\n"
...
_SCAN_SYS = (
    ...
    "[Производитель] — д.в., норма; <одно отличие>». Отличие — ТОЛЬКО то, что выделяет препарат среди "
    "остальных: почвенный/довсходовый или листовой, скорость действия, шире/уже спектр, группа по "
    "механизму действия (для чередования), ограничения (напр. только на устойчивых гибридах). НЕ пиши "
    "общие для всех фразы («эффективен против двудольных», «норма зависит от культуры и стадии») — они "
    "не различают препараты и не нужны. НЕ ранжируй эффективность брендов. Плюс 1 агроприём. Если блока "
    "нет — назови д.в. и попроси уточнить культуру.\n"
