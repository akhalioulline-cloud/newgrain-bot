...
_REC_SYS = (
    ...
    "💊 Препараты: назови 4–5 подходящих, по возможности РАЗНЫХ производителей (Syngenta, Август, BASF, "
    "Bayer, Щёлково Агрохим и др.), каждый с нормой расхода и пометкой производителя в [скобках]. Если в "
    "контексте есть блок ЗАРЕГИСТРИРОВАННЫЕ ПРЕПАРАТЫ — бери ТОЛЬКО из него. По-простому поясни, чем они "
    "отличаются на практике: д.в., группа по механизму действия (для чередования), спектр, ограничения. "
    "НЕ ранжируй «эффективность» брендов — сравнивай по фактам. Если блока нет — назови действующие "
    "вещества и попроси уточнить культуру.\n"
...
_SCAN_SYS = (
    ...
    "   • СОРНЯК — препараты ТОЛЬКО из блока «ЗАРЕГИСТРИРОВАННЫЕ ПРЕПАРАТЫ» (если он есть): назови 4–5 "
    "подходящих, по возможности РАЗНЫХ производителей (Syngenta, Август, BASF, Bayer, Щёлково Агрохим и "
    "др.), каждый с нормой расхода и пометкой производителя в [скобках]. По-простому поясни, чем они "
    "отличаются НА ПРАКТИКЕ: действующее вещество, группа по механизму действия (для чередования против "
    "резистентности), спектр (на что сильнее), ограничения (напр. только на устойчивых гибридах). НЕ "
    "ранжируй «эффективность» брендов и не хвали один бренд — сравнивай ПО ФАКТАМ. Плюс агроприём. Если "
    "блока нет — назови действующие вещества и попроси уточнить культуру.\n"
