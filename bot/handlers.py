...
# One-shot "what's new" broadcast: an admin runs /announce in the team group and the
# bot posts these (one per message) right there.
_ANNOUNCEMENTS = [
    "📝 <b>Запись обработок голосом или текстом</b>\n"
    ...
    "🌿 <b>Подсказка по сорняку на фото</b>\n"
    ...
    "✅ <b>Проверка фото старшим агрономом</b>\n"
    ...
    "💬 <b>Спросите бота про поле — как у коллеги</b>\n"
    ...
    "📋 <b>Задания машин — в один шаг</b> (для оператора)\n"
...
async def cmd_announce(message: Message, user) -> None:
    if not _is_admin(user):
        return
    await message.answer("📣 <b>Что нового в боте Flagleaf</b>", parse_mode="HTML")
    for text_msg in _ANNOUNCEMENTS:
