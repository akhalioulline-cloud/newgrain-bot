...
**Chat responsiveness+quality pass (1 Jul 2026).** Three changes to the ИИ-агроном (bot/agro_chat.py, api/main.py, web/app/assistant.html): (1) **Model 5.1** — assistant now on `settings.yc_chat_model = "yandexgpt/rc"` (= yagpt-5.1-2025-08); verified live that plain `yandexgpt` and `yandexgpt/latest` BOTH still resolve to 5.0/09.02.2025, so `/rc` is the only way to 5.1. Kept separate from `yc_translate_model` (parse/translate stay on stable "yandexgpt"). rc = release-candidate/preview, revert by setting yc_chat_model back to "yandexgpt". (2) **Streaming** — new `POST /api/chat/stream` (StreamingResponse of UTF-8 text deltas, header `X-Accel-Buffering:no` so nginx flushes; grounding awaited first, then completion streams). agro_chat split: `assemble_prompt` (async grounding) + `stream_complete`/`_complete_stream` (sync delta generator — YandexGPT streams CUMULATIVE text, so we diff by prefix). Assistant UI reads the ReadableStream and renders word-by-word. TTFB ~1.5s vs ~3.7s total. Old `/api/chat` kept for compat; Telegram bot uses `answer()` (non-stream) — both share `_assemble`, so all paths got 5.1. (3) **Killed the double LLM call** on protection questions: `_lexicon_extract` (compact crop `_CROP_LEX` + weed `_WEED_LEX` → {crop,target,weed_class}) resolves common cases with NO llm; misses fall back to `settings.yc_extract_model` — tested lite here and it MIS-classified «падалица рапса в пшенице» as злаков (should be двудольн; pro correct), so kept it on the pro model "yandexgpt" (the lexicon already removes the double-call on common Qs, so the rare fallback keeps correctness). Padalitsa/самосев deliberately deferred to the LLM (weed class is nuanced). SW v20→v21. Config defaults mean no .env change needed.
