...
**Scouting videos now go through Almas's review too (1 Jul 2026, his request).** Photos already had the chief-agronomist gate; Almas asked that agronomists' scouting *videos* undergo his verification "before being finally pulled." Built: a junior's video lands at `pending_review` (`/api/scout-video`); after the collector transcribes the narration it calls `bot/video_review.py::send_video_review_for`, which pushes the chief a card = transcript + a 7-day presigned watch link + Подтвердить/Отклонить. Approve → `stored` (feeds /plan); reject → `rejected`. `on_review` (handlers.py) is now category-aware via `_approved_status()` (scouting→stored, else ready_for_labeling) and gained a `rev:no` reject branch. `get_field_observations` excludes `pending_review` so an unverified video can't reach a plan before approval. No chief configured → auto-finalize to stored (never stranded). Chief/admin uploads stay direct. Scope decision: **videos only** — scouting PHOTOS stay bypassed (a 30-photo pass would flood him); videos are few (≤3/agronomist). Announce #18. Ties to [[newgrain-labeling-pipeline]] (the 'stored' terminal status) and [[newgrain-pilot-v2]].

**Review moved OUT of the «Проверка» tab and INTO the assistant chat + a morning nudge (4 Jul 2026, founder ask).** The standalone `review.html` tab is gone (now a redirect stub → `/app/assistant.html?review=1`; the tab injection removed from index.html `enterApp` and from assistant.html boot). Instead, when a chief opens the merged Ассистент (assistant.html), `loadReview()` fetches `/api/review/pending`; if non-empty it posts an in-chat **🌅 morning digest** bubble («N фото + M видео» → «✅ Подтвердить все» [with an inline confirm] / «Проверить по одной» → per-item cards: media + field/submitter/species + **✅ Верно / ✏️ Исправить / ⏭ Пропустить**, next card after each action). All actions hit the SAME `/api/review/decide` (approve + optional field/category/species/comment corrections). A chief can also summon it anytime by typing (`REVIEW_RE`: «покажи материалы на проверку» etc.). The **morning nudge** = new `bot/morning_review.py`, cron **05:00 UTC = 08:00 MSK** (`docker compose run --rm bot python -m bot.morning_review`), sends each active chief a Web Push + a Telegram DM (link to `?review=1`) ONLY when the queue is non-empty; single-farm pilot → global pending count. Reuses `get_chief_agronomists`, `get_pending_reviews`, `send_push`, and a one-off relay Bot. SW v39. Verified: script runs clean + no-ops on empty queue; cron installed. Supersedes the separate-tab design just below. Builds on [[newgrain-pwa]] (Web Push infra) + [[newgrain-motivation-no-gamification]].

**Review is now in the app too, not just Telegram (1 Jul 2026).** Almas asked to review photos AND videos in the PWA. Built an in-app review inbox: `web/app/review.html` (a chief-only "✅ Проверка (N)" tab, injected in enterApp of index.html + on assistant.html; badge from `/api/review/count`). Endpoints in api/main.py: `/api/review/count`, `/api/review/pending` (photos + scouting videos at pending_review for the chief's farm, each with a fresh 7-day presigned media link + `is_video` flag; `<img>` for photos, `<video controls>` for videos), `/api/review/decide` (approve/reject + optional inline corrections field/category/species/comment). The decision path is SHARED with the Telegram buttons via new `bot/review_actions.py` (`approved_status` + `notify_submitter_decision`) — same status routing (scouting→stored, photo→ready_for_labeling; reject→rejected) and same submitter notice wherever Almas acts. `on_review` refactored to use the shared helpers + gained a stale-card guard (skips if status≠pending_review, so an app-approval doesn't get re-flipped by a lingering Telegram card); `/api/review/decide` has the same guard. New db helpers `get_pending_reviews`/`count_pending_reviews`. SW bumped v19→v20 (review.html in the shell). Verified on prod: chief=Алмас Касумов (farm 1), 14 pending photos all with working media links. Extends [[newgrain-pwa]].
